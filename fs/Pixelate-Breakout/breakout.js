...
function startGame() {
    ...
    GO.bricksGrid = new makeGrid(GO.bricksTable, GO.effectsDim, 
        {
            ...
            ColsRows:[10,7], pxMult:[15,4], varyColors:brickColors 
        } );

    GO.brickCount = GO.bricksGrid.count;

    GO.brickWall = new makeBrickWall(
        { regular:true, nBricksPerRow: 10, numRows:15, firstRow:0, numRowsPerBrick:4} )
    ...
    GO.fps = 50;  //let's try 60 frames per second
...
function ivecAdd( a1, num, trunc=true) {  //integer vector (of 2) addition
...
class Sprite {
    ...
    updatePosition(newPosXY_untruncated) {
        //update old position to background color
        //update new position to PixelMap

        const newPosXY = [ Math.trunc(newPosXY_untruncated[0]), Math.trunc(newPosXY_untruncated[1]) ]
        let oldPixels = [];
        let newPixels = {};
        ...
        for (let y=0; y<this.maxDim[1]; y++) {
            ...
            for (let x=0; x<this.maxDim[0]; x++) {
                ...
                const isBgColor = isBackgroundColor(this.PixelMap[y][x]);
                //create a compound key of the coordinate so we can lookup later
                newPixels[newPos.join()] = isBgColor ; // isBackgroundColor(this.PixelMap[y][x]);

                let Pixel = GO.mainGrid.tdByXY[newPos[1]][newPos[0]];
                let oldPixel = GO.mainGrid.tdByXY[y+Math.trunc(this.posXY[1])][x+Math.trunc(this.posXY[0])];
 ...
 */
  ...
  constructor() {
    ...
    for (let y = 0; y < this.dim[1]; y++) {
      ...
      for (let x = 0; x < this.dim[0]; x++) {
        ...
        if (ii === 0) {
          color = GO.backgroundColor; //'rgba(0,0,0,0)' ; //
        } else {
  ...
  hitWall(pos) {
    ...
    for (let i = 0; i < objects.length; i++) {
      if (
        objects[i].tagName === "TD" &&
        objects[i].offsetParent.id === "bricks"
      ) {
  ...
  newPosition() {
    ...
    if (paddleOffset > -1e5) {
      ...
      const ratio = oldSpeed / this.speed;
      ...
      console.log(paddleOffset, this.dv, ratio);
    ...
    if (newPos[1] + this.maxDim[1] > GO.mainDim.height) {
      ...
      setTimeout(() => {
        GO.colorEffects.style.backgroundColor = "rgba(0,0,0,0)";
      }, 100);
    ...
    newPos[1] = Math.min(
      GO.mainDim.height - this.maxDim[1],
      Math.max(0, newPos[1])
    );
...
class Paddle extends Sprite {
    ...
    constructor() {
        ...
        if ( this.dim[0]/2 === Math.trunc( this.dim[0]/2) ) {
            this.dim[0] ++
        }
...
/*         for (let y=0; y<this.dim[1]; y++ ) {
        ...
        this.posXY = [50,88];
        this.updatePosition( [51,88] );  //make sure the updatePosition is not same so it renders the first
...
class makeGrid {
    ...
    constructor(table, dim, options={initColor:'rgba(100,100,100,.8)'}) {  
        ...
        if (options.useMainPxSize) {
            ...
            this.dim = dim;
            this.PixelDimXY = [ divWidth/dim.width, .9*divHeight/dim.height ];
        }
        ...
        if ( options.addEventListener ) {
            ...
            table.addEventListener('mousemove', function(event){
                ...
                if ( Array.isArray(event.target.mousePixelPos))
...
class makeBrickWall {
  ...
  constructor(layoutOptions) {
    if (layoutOptions.regular) {
      const { nBricksPerRow, numRows, firstRow, numRowsPerBrick } =
        layoutOptions;

      this.pixelsPerBrick = Math.trunc(GO.mainDim.width / nBricksPerRow);

      for (let rowNum = 0; rowNum < numRows; rowNum++) {

        let row = []
        for (let brickNum = 0; brickNum < nBricksPerRow; brickNum++) {
          let brick = [];

          for (let brickRow = 0; brickRow < numRowsPerBrick; brickRow++) {

            for (let pixel = 0; pixel < this.pixelsPerBrick; pixel++) {

              const pixelNum = this.pixelsPerBrick * brickNum + pixel;
              const pixelRow = numRowsPerBrick * rowNum + brickRow
              let thisTD = GO.mainGrid.tdByXY[firstRow + pixelRow][pixelNum];
        ...
        this.BrickPixels.push(row)

      }

      console.log(this.BrickPixels)
    }
  }
}